...
sigma2 = 1;
step = 0.01; 
Zrange = 0:step:6.28;                  % For Z>=0
Thetarange = -pi:step:pi;           % -pi<= Theta <=pi
Zh = hist(Z, Zrange); 
Thetah = hist(angle(z), Thetarange);

fr_approx = Zh/(step*N);
frz = (Zrange/sigma2).*exp(-(Zrange.^2)/(2*sigma2));

figure
subplot(2,1,1)
plot(Zrange, fr_approx,'r', Zrange, frz,'b');
...
frtheta = 1/(2*pi)*ones(size(Thetarange));


subplot(2,1,2)
plot(Thetarange, fr_approx,'r')
hold on
plot(Thetarange, frtheta,'b');
legend('Simulated PDF', 'Analytical PDF')
title('PDF P(theta)')
axis([-pi pi 0.14 0.18])
grid; 
ztest = frz.*exp(1i*frtheta);
[ Xnorm, Averagepower, AbsolutePower, Power, RMS, Simga2, DCGain, PowerGain ] = SignalInfo( z );
temp = 1;