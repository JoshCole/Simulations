...
colLit=hsv(AntennaVariation+1);
count = 1;
figure
theoryBer_nRx1 = 0.5.*(1-1*(1+1./EbNo_lin).^(-0.5)); 
semilogy(EsNo_dB,theoryBer_nRx1,'--','col',colLit(AntennaVariation+1,:))
hold on
leg{count} =['ZF',' Simulation Nrx = Ntx = NTx'];
count = count+1;
...
%                     figure
%                     subplot(2,1,1)
%                     plot(abs(y(1,:)), 'col',colList(1,:))
%                     hold on
%                     plot(abs(y(2,:)), 'col',colList(2,:))
%                     plot(abs(sumy), 'col',colList(3,:))
%                     title('Time')
%                     grid on
%                     legend('y1','y2', 'y sum')
%                     subplot(2,1,2)
%                     plot(20*log10(abs(fft(y(1,:)))), 'col',colList(1,:))
%                     hold on
%                     plot(20*log10(abs(fft(y(2,:)))), 'col',colList(2,:))
%                     plot(20*log10(abs(fft(sumy))), 'col',colList(3,:))
%                     title('Frequency')
%                     grid on
%                     legend('y1','y2', 'y sum')
...
%      hold on
     ...
     semilogy(EsNo_dB,BitError,'-','col',colLit(Ant,:))
     hold on
     leg{count} =['MMSE',' Simulation Nrx = Ntx = ',num2str(NTx)];
     count = count+1;
