...
for i = 1:length(FsArray)
    ...
    n = length(pulseShaping);
    t = (-(n-1)/2:(n-1)/2)/Fs;
%     t = (0:n-1)/Fs;
    plot(t,pulseShaping);
    grid on
%     axis([-(n-1)/2 (n-1)/2 -0.05 0.2])
    IndexZ = find(pulseShaping == 0);
    ZeroCross = pulseShaping(IndexZ);
    t = t(IndexZ);
    hold on
    plot(t,ZeroCross,'ro');
    [ h ] = RaisedCosine( 0.22, 12, Fs )/Fs;
    ...
    xlabel('T');
    ylabel('Amplitude');
    title('Impulse Response of Pulse Shaping Filter');
    pause
   ...
   [Peak, index] =max(pulseShaping);
    ...
    figure
    subplot(2,1,1);plot(t,real(PulseShapedResponse2));
    hold on
    ...
    legend('Pulse Shaped Signal','Sampled Signal')
    ...
    t = (0:length(Filteredsig2)-1)/Fs;
    plot(t,real(Filteredsig2),':g');
    axis([0 20 -1.25/Fs 1.25/Fs])
    xlabel('Time (sec)');
    ylabel('Amplitude');
    title(['Analogue Filtered, Over Sampled Fs = ',num2str(Fs)]);
    legend('Pulse Shaped Signal','Sampled Signal', 'Analog filtered Pulse Shaped Signal')
    pause
