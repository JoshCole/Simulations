...
PacketSizeArray = [2^8];
NumberOfPacketsArray = 100;2^20./PacketSizeArray;
NumPacketSizes = length(PacketSizeArray);
NyquistRateArray = [2, 4, 8];
NumNyquist = length(NyquistRateArray);
...
% -----------------------
VariationVariable = NumNyquist;%NumPacketSizes;%NumPacketSizes;%NumBitError;      % Variable Intorduction of errors, variation in packet size etc
...
MBLER = zeros(NumEsNo,NumM+VariationVariable-1);
        ...
        for m = 1:NumM
            ...
            for i = 1:NumEsNo
                ...
                for ii = 1:NumberOfPackets
                    ...
                    TxFilter = conv(FilterRRC*Fs,TxUpSample);
                    powerFill = mean(abs(TxFilter).^2);
                    % Filter Peak
                    [Peak, index] =max(FilterRRC);
                    index = index-1;

                    TxFilter = TxFilter.'; 
                                       
                    [ RxSymbol, Noise , No, NoiseVar ] = TransmitSymbol( TxFilter, Es, EsNo_lin(i), Fs );
                    [ RxSymbol ] = AddBurstError( RxSymbol, Es, No, ErrorBurstSymbolSize, NumberOfBurstError );
                    
                    powerRx = mean(abs(RxSymbol).^2);
                    % Matched Filter
                    % --------------
                    rxAWGN = conv(FilterRRC,Noise);
                    FilterOutput = conv(FilterRRC,RxSymbol);
                    powerRx = mean(abs(FilterOutput).^2);
                    FilterOutput = FilterOutput+rxAWGN;
                    powerRx = mean(abs(FilterOutput).^2);
                    index = 2*index ;
                    FilterOutputRRC2 = FilterOutput;

                    RxSymbol = FilterOutputRRC2(index+1:Fs:index+Fs*numSymbols);

                    powerRRC = mean(abs(RxSymbol).^2);
%                     plot(RxSymbol,'o');
            ...
            MBitError(:,m+j) = BitError(1,:);
            MSymbolError(:,m+j) = SymbolError(1,:);
            MBLER(:,m+j)= CRCER(1,:);
            MCRCERTotal(:,m+j)= CRCERTotal(1,:);
...
NewLeg = [MLegendArray,MLegendSimArray];
            ...
            figure
            semilogy(EbNo_dB,MBitErrorTheo,'m');
            hold on
            semilogy(EbNo_dB,MBitError,':o');
            grid on
    
            title(MTitle);
%             legend('Bit Error Theory', 'Bit Error', 'BLER Theory', 'BLER
%             Simulated');
            xlabel('Eb/No, dB');
            ylabel('Bit Error Rate');
            ...
            figure
            semilogy(EsNo_dB,MSymbolErrorTheo);
            hold on
            semilogy(EsNo_dB,MSymbolError,':o');
            grid on
...
%             Simulated');
            xlabel('Es/No, dB');
            ylabel('Symbol Error Rate');
%             axis([min(EsNo_dB) max(EsNo_dB) 10^-4 10^-0 ])
