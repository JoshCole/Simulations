...
%
%               FrameSize   = The Number of symbols in a frame

No = (Es./EsNo_lin);    % Noise Spectral Density
NoiseVar = (No/2);      % Noise Variance
AWGN = (((randn(1,FrameSize))+1i*(randn(1,FrameSize)))/(sqrt(2)))/sqrt(Esnorm);     % Normalised AWGN in respect to Esnorm
Noise = (sqrt(No).*AWGN);       % Noise to be induced to Signal
